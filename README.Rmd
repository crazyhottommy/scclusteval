...
boostrapping the cells and provide many visualization methods for comparing clusters.
...
`FindClusters` works on `Seruat` object. If you have a data matrix, take a look at `scran` package, make a `SingleCellExperiment` object and read through https://bioconductor.org/packages/release/workflows/vignettes/simpleSingleCell/inst/doc/work-2-umis.html#9_clustering_cells_into_putative_subpopulations

Note that `buidSNNGraph` works on data matrix as well.
```
library(scran)
...
table(sce$Cluster)
```
The parameter `k` which specifies the number of nearest neighbors has a great effect on the number
of clusters.
